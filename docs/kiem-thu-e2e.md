...
| C9 | Bước `click`/`theoChữ` với một từ đang hiện trên màn hình | Click đúng tâm đoạn chữ | `RG-13` `RG-15` | **Đạt sau khi sửa** — lần đầu click `(356,551)` = tâm **cả dòng** `"ZUKAMI QWERTY"`, không phải tâm chữ cần tìm; lệch 83 point (xem `RG-15`). Chạy lại trên trang bia, nơi mỗi từ được bọc `<span>` riêng nên trang **tự khai toạ độ của từng từ**: Auto Click click `(138,716)` = đúng tâm `word-zukami`, trong khi tâm cả dòng là `(404,716)` — cách nhau 266 point, không thể nhầm lẫn. Trang xác nhận cú click rơi vào `word-zukami` |
| C10 | Lặp C9 với chữ viết hoa/thường khác đi | Vẫn tìm thấy | `RG-14` | **Đạt** — Kịch bản đi tìm `"zukami"` viết thường trong khi trên màn hình là `"ZUKAMI"` viết hoa: vẫn click đúng `word-zukami@(138,716)` |
...
| E5 | Đổi Ảnh mẫu của một Bước vài lần, đếm tệp trong `templates/` | Tệp cũ bị dọn, thư mục không phình | `ST-12` | **Đạt** — nhân bản `I1` rồi chụp lại Ảnh mẫu **ba lần** vào ba vùng khác nhau. Sau mỗi lần `templates/` có **đúng một** tệp, tên đổi mỗi lần (`S3.png` → `50DD6B9E.png` → `2956631F.png` → `B93CF474.png`), tệp cũ biến mất ngay. `ScenarioStore.save` gọi `removeUnused(keeping:)` sau mỗi lần ghi nên không có tệp mồ côi nào đọng lại.<br><br>Nhân thể xác nhận `RG-23` đầu-cuối: vùng khoanh cuối cùng là `(60,520,220,140)`, đệm `min(160, max(48, 220/2)) = 110`, nở ra `(-50,410,440,360)`, cắt theo màn hình còn **`(0,410,390,360)`** — đúng con số nằm trong `scenario.json`. Và `ST-3` ở chiều nhân bản cũng đúng: bản sao của `I1` có sẵn `S3.png` trong `templates/` riêng của nó |
