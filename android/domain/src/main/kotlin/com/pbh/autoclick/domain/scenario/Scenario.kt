...
data class Scenario(
    ...
    /** RC-28: whether anything in here needs a schema an older build could not read. */
    val usesRecognition: Boolean get() = steps.any { it.effectiveSearch != null || it.guard != null }

    /**
     * RC-27: every Template this Scenario still refers to, so the rest can be swept.
     *
     * Reads `search` rather than `effectiveSearch` on purpose. A search left on a Step whose
     * Action stopped using a Target is ignored at run time (`RC-22`) and is still the user's
